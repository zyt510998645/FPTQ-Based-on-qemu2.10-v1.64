...
struct TranslationBlock {
    ...
    uint16_t size;      /* size of target code for this block (1 <=
                           size <= TARGET_PAGE_SIZE) */ //tb的大小最大为页大小--可能是4k
    uint16_t icount;
...
#define CF_IGNORE_ICOUNT 0x40000 /* Do not generate icount code */
    ...
    void *tc_ptr;    /* pointer to the translated code */ //这里用的是code而不是tranlated instructions。 所以应该一个段代码。
    uint8_t *tc_search;  /* pointer to search data */
    /* original tb when cflags has CF_NOCACHE */ //这里的nocache应该是指这里的代码内容还没有被翻译过？
     ...
     * two of such jumps are supported. //下面的数据是为了能够从当前tb跳转到另一个tb，实现这个步骤可以使用本地的间接或直接跳转命令来实现。/只有其中的两种的jump方式能够支持。
     */
...
#define TB_JMP_RESET_OFFSET_INVALID 0xffff /* indicates no jump generated */
     ...
     * and 2 means that this is a pointer back to the target TB of this list. //每一tb都有一个循环队列，包含了很多tbs能够跳转到当前tb。jum_list_first指向了循环队列的第一tb，jmp_list_next 指向了下一个tb。每一个tb拥有两种jump方法，所以他们就有两种的跳转列表。jmp_list_first 和jmp_list_next都是4 bytes对齐的并且指向一个tb结构，但是它们最低的2bit内容用于对决定被指向的tb的哪些数据位域能够贯穿使用在其他在该list上的tb。
     */
...
/* Called with tb_lock held.  */ //这里tb_lock是为了保证原子操作
                               ...
                               TranslationBlock *tb_next)
{
    ...
    tb_set_jmp_target(tb, n, (uintptr_t)tb_next->tc_ptr); //其实就是last_tb跳转到tb

    /* add in TB jmp circular list */
    tb->jmp_list_next[n] = tb_next->jmp_list_first; //这里存疑。这是怎么循环转移回去的？
