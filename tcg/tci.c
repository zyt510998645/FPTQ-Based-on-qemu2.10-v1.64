/*
 * Tiny Code If(INTERVAL_RDD || INTERVAL_RDD_SAMPLE)
                    {
                        LOG_QEMU_RDD = 1;
                    }
nterpreter for QEMU
...
TypeU64 SET_NUM_LIST[4096] = {0};
TypeU64 Sample_RDD[4096]={0},Sample_SDD[4096]={0};//ZYT 3-8
TypeU64 Sample_RDD_taddr[4096]={0},Sample_SDD_taddr[4096]={0};//ZYT 3-8
TypeU64 Sample_RDD_flag[4096]={0},Sample_SDD_flag[4096]={0};//ZYT 3-8
TypeU64 HIS_sample_RDD[1025];
TypeU64 HIS_sample_SDD[1025];
TypeU64 Sample_RDD_LS = 0, Sample_SDD_LS = 0; 
//#if LOG_QEMU_RDD      
#include "qemu/rb_tree.h"
TypeU64 HIS_RDD[1025];
TypeU64 RDD_LS = 0;
static RBRoot * Temp[SET_NUM] = {NULL};
//#endif
//#if LOG_QEMU_SDD
#include "qemu/list_sdd.h"
TypeU64 HIS_SDD[1025];
TypeU64 SDD_LS = 0;
//#endif
//*********************************ZYT
...
//***************************************************ZYT
    //printf("CHECK 7\n");//ZYT
/*
    static bool init_check_flag = true;
    if(init_check_flag == true)
    {
	assert(SAMPLE_RATE >= 1);//sampling rate must >= 1;
...
if(LOG_QEMU_RDD)
{
    ...
    if(first_flag == true)
    {
        TypeU64 just_for_init_i = 0;
        assert(GET_SET <= SET_NUM);//判断是否超出最大set数上限！
        while(just_for_init_i < GET_SET)
        {
            Temp[just_for_init_i++] = create_rbtree(); //ZYT
        }
        assert(just_for_init_i == GET_SET);//断定空间是否足够
        //printf("CHECK!:%lu %lu %lu\n",GET_SET,GET_SET_BIT,CACHE_LINE_BIT);
        first_flag = false;
    }
}
//#endif
...
//*****************************************************ZYT
            LOG_ASSOCIATE(SET_NUM_LIST[Set_Id(taddr)]);
if(LOG_QEMU_RDD)
{
    if(INTERVAL_RDD_SAMPLE)
	{
	    getSampleRdd(Sample_RDD, Sample_RDD_taddr, (TypeU64)taddr, HIS_sample_RDD, Sample_RDD_flag, &Sample_RDD_LS);
	}
    if(INTERVAL_RDD)
	{
	    insert_rbtree(Temp[Set_Id(taddr)], (TypeU64)(taddr)>>CACHE_LINE_BIT, RDD_LS++, HIS_RDD);
	}
}
if(LOG_QEMU_SDD)
{
    if(INTERVAL_SDD)
	{
            search_sdd((TypeU64)Set_Id(taddr), (TypeU64)(taddr)>>CACHE_LINE_BIT ,SDD_LS++, HIS_SDD);
	}
    if(INTERVAL_SDD_SAMPLE)
	{
	    getSampleSdd((TypeU64)taddr, HIS_sample_SDD, Sample_SDD, Sample_SDD_taddr, Sample_SDD_flag, &Sample_SDD_LS);
	}
}
           if(LOG_LS_TRACE)
           {
            LOG_TRACE_PRINT((TypeU64)(taddr));
           }
//******************************************************
