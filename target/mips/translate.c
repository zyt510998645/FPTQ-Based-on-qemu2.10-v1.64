...
static void gen_store_fpr64(DisasContext *ctx, TCGv_i64 t, int reg) //这里是从目标代码变成tcg代码。
{
    if (ctx->hflags & MIPS_HFLAG_F64) {
                   ...
                   int rt, int base, int16_t offset)
{
    ...
    if (rt == 0 && ctx->insn_flags & (INSN_LOONGSON2E | INSN_LOONGSON2F)) {//rt是目的操作数。
    ...
    t0 = tcg_temp_new();
    gen_base_offset_addr(ctx, t0, base, offset); //这里的t0就是计算完成的地址。
/******jkc*******/
...
void gen_intermediate_code(CPUState *cs, struct TranslationBlock *tb) //产生中间代码。换言之就是从guest（mips）产生对应的tcg 代码。
{
    ...
    pc_start = tb->pc;
    next_page_start = (pc_start & TARGET_PAGE_MASK) + TARGET_PAGE_SIZE; //开始计算下一个page的起始地址。//taget_page_mask=0xfffff000也就是屏蔽的是12bit也就是4k地址大小。 
    ctx.pc = pc_start;
    ctx.saved_pc = -1;
    ...
    gen_tb_start(tb);
    while (ctx.bstate == BS_NONE) { //完成一个tb的翻译肯定需要一个执行循环作为保障。 //we go out of the tb without any branches nor exceptions
        tcg_gen_insn_start(ctx.pc, ctx.hflags & MIPS_HFLAG_BMASK, ctx.btarget); //这里应该是开始翻译指令。
        num_insns++;
        ...
        if (num_insns == max_insns && (tb->cflags & CF_LAST_IO)) {
            gen_io_start(); //如果总的指令数量已经到达了最大的翻译指令数目，同时，标志为最后的io,那么就开始进入到io的指令翻译。
        }

        is_slot = ctx.hflags & MIPS_HFLAG_BMASK;
        if (!(ctx.hflags & MIPS_HFLAG_M16)) {
            ctx.opcode = cpu_ldl_code(env, ctx.pc);
            insn_bytes = 4; //这里的指令大小是4个bytes,也就是32bit.
            decode_opc(env, &ctx);
        } else if (ctx.insn_flags & ASE_MICROMIPS) { //按照不同的指令规格或者不同的指令种类来进行翻译。
            ctx.opcode = cpu_lduw_code(env, ctx.pc);
            insn_bytes = decode_micromips_opc(env, &ctx); //这里的bytes大小是译码的mips的微操作数量。
        } else if (ctx.insn_flags & ASE_MIPS16) {
            ctx.opcode = cpu_lduw_code(env, ctx.pc); //这里获得是opcode。也就是指令的种类是已经知道。
        ...
        if (is_slot) {
            gen_branch(&ctx, insn_bytes); //这里就判定是不是branch指令了。
        }
        ctx.pc += insn_bytes; //这里的pc开始更新，按照已经翻译的指令的大小来进行指令的递进。
        ...
        if (ctx.pc >= next_page_start) { //这里是判定有没有超界。
            break;
        }

        if (tcg_op_buf_full()) { //如果译码的buffer满了，也会退出。
            break;
        }

        if (num_insns >= max_insns) //这里是翻译了的代码数量不能超过某个值。
            break;

        if (singlestep) //如果是单步调试的模式在完成一次翻译后也会退出。
            break;
    }
...
done_generating:
    ...
    tb->size = ctx.pc - pc_start; //这里计算的整个tb的大小。
    tb->icount = num_insns;
