...
static TranslationBlock *tb_alloc(target_ulong pc)
{
    ...
    tb = tcg_tb_alloc(&tcg_ctx);//按照tcg_ctx来分配tb，但是tcg_ctx没有在当前函数定义。 TCGContext tcg_ctx 产生tcg指令的上下文环境
    if (unlikely(tb == NULL)) {
        return NULL;
    }
    ctx = &tcg_ctx.tb_ctx;
...
TranslationBlock *tb_gen_code(CPUState *cpu,
                              ...
                              uint32_t flags, int cflags) //这里的pc和cs_base就决定了翻译的位置。
{
    CPUArchState *env = cpu->env_ptr; //这里指的是mips的cpu状态。
    TranslationBlock *tb;
    tb_page_addr_t phys_pc, phys_page2; //物理页框和物理指针。
    target_ulong virt_page2; //虚拟页框
    tcg_insn_unit *gen_code_buf; //tcg 指令的大小，或者说一条tcg指令应该多长。1个byte，2个bytes或者4个bytes等等。
    int gen_code_size, search_size; //产生tcg代码
#ifdef CONFIG_PROFILER
    int64_t ti;
#endif
    assert_memory_lock();

    phys_pc = get_page_addr_code(env, pc); //直接返回PC值。
    if (use_icount && !(cflags & CF_IGNORE_ICOUNT)) {
        cflags |= CF_USE_ICOUNT;
    } //是否忽略了icount，如果需要icount但是又忽略了icount，那么就置位。

    tb = tb_alloc(pc); //按照pc值来分配tb模块,这个的pc个人认为是程序执行的pc。
    if (unlikely(!tb)) { //likely是为了告诉编译器，这部分代码是很容被执行到的，在编译的时候就会放到代码的前半段。而unlikely函数正好相反。:
 buffer_overflow:
        /* flush must be done */
        tb_flush(cpu); //清空CPU所有的状态。
        mmap_unlock(); //解锁mmap，应该是物理地址到虚拟地址映射的机制。
        /* Make the execution loop process the flush as soon as possible.  */
        cpu->exception_index = EXCP_INTERRUPT; //异常函数索引被更换为异常中断。
        cpu_loop_exit(cpu); //停止执行。
    }

    gen_code_buf = tcg_ctx.code_gen_ptr; //被翻译好的指令。
    tb->tc_ptr = gen_code_buf;
    tb->pc = pc; //当前的pc就是tb的pc。
    tb->cs_base = cs_base; //当前的段地址就是tb的段地址。
    tb->flags = flags;
    tb->cflags = cflags;
    tb->trace_vcpu_dstate = *cpu->trace_dstate;
    tb->invalid = false;
//总体来说，不论当前pc找不找到tb，重新按照当前的pc值分配一个tb。
#ifdef CONFIG_PROFILER 
    ...
    tcg_func_start(&tcg_ctx); //这里就是tcg函数的入口,做了一些清空数据结构的函数。 //这里就是一个tcg block的接口

    tcg_ctx.cpu = ENV_GET_CPU(env);
    gen_intermediate_code(cpu, tb); //这里开始从原始代码转化成中间代码。也就是从mips代码转化成tcg指令。 //一个tb只能代表一个guest的指令。
    //printf("CHECK 1\n");//ZYT
    tcg_ctx.cpu = NULL;
    ...
    gen_code_size = tcg_gen_code(&tcg_ctx, tb); //这里是确实开始译码的位置。
    //printf("CHECK 2\n");//ZYT 
