...
long unsigned int INTERVAL_RDD=20;
long unsigned int INTERVAL_RDD_SAMPLE=20;
long unsigned int INTERVAL_RDD_SAMPLE_RATE=1000;
unsigned int LOG_QEMU_RDD=0;
long unsigned int INTERVAL_SDD=20;
long unsigned int INTERVAL_SDD_SAMPLE=20;
long unsigned int INTERVAL_SDD_SAMPLE_RATE=1000;
unsigned int LOG_QEMU_SDD=0;
unsigned int LOG_LS_TRACE=0;
//unsigned int LOG_QEMU_SAMPLE=0;
long unsigned int SAMPLE_RATE=1000;
//long unsigned int SET_NUM=65537;
long unsigned int L1D_SIZE=64;
long unsigned int CACHE_LINE=64;
long unsigned int ASSOCIATE=1024;
long unsigned int GET_SET=0;
long unsigned int GET_SET_BIT=0;
long unsigned int CACHE_LINE_BIT=0;
...
static const struct qemu_argument arg_table[] = {
    ...
    //{"l1dc",       "QEMU_L1DCS",       true, handle_arg_version,
    // "cs cl assoc","L1 DCache size'cs'KB;CACHE_LINE_SIZE 'cl'Byte;ASSOCIATE 'assoc'"},//ZYT
...
int main(int argc, char **argv, char **envp)//ZYT
{
...
//ZYT
    int i_zyt=0;
    for(i_zyt=0;i_zyt<argc;i_zyt++)
    {
	if(strcmp(argv[i_zyt],"-RSDD")==0)
	{
	    INTERVAL_RDD = atoi(argv[i_zyt+1]);
	    INTERVAL_RDD_SAMPLE = atoi(argv[i_zyt+2]);
            INTERVAL_RDD_SAMPLE_RATE = atoi(argv[i_zyt+3]);
		if(INTERVAL_RDD || INTERVAL_RDD_SAMPLE)
		    {
			LOG_QEMU_RDD = 1;
		    }
	    INTERVAL_SDD = atoi(argv[i_zyt+4]);
	    INTERVAL_SDD_SAMPLE = atoi(argv[i_zyt+5]);
            INTERVAL_SDD_SAMPLE_RATE = atoi(argv[i_zyt+6]);
                if(INTERVAL_SDD || INTERVAL_SDD_SAMPLE)
                    {
                        LOG_QEMU_SDD = 1;
                    }
	}

        if(strcmp(argv[i_zyt],"-CACHE")==0)
        {
            //SET_NUM = atoi(argv[i_zyt+1]);
            L1D_SIZE = atoi(argv[i_zyt+1]);
            CACHE_LINE = atoi(argv[i_zyt+2]);
            ASSOCIATE = atoi(argv[i_zyt+3]);
	    GET_SET = Get_Set(L1D_SIZE, CACHE_LINE, ASSOCIATE);
	    GET_SET_BIT = Get_Set_Bit(GET_SET);
	    CACHE_LINE_BIT = Cache_Line_Bit(CACHE_LINE);
	    //SET_ID = Set_Id(x, GET_SET, CACHE_LINE_BIT);
        }


/*
        if(strcmp(argv[i_zyt],"-RDD")==0)
        {
	    LOG_QEMU_RDD = 1;
            INTERVAL_RDD = atoi(argv[i_zyt+1]);
        }
        if(strcmp(argv[i_zyt],"-SDD")==0)
	{
	    LOG_QEMU_SDD = 1;
            INTERVAL_SDD = atoi(argv[i_zyt+1]);
	}
        if(strcmp(argv[i_zyt],"-SAMPLE")==0)
	{
	    LOG_QEMU_SAMPLE = 1;
            SAMPLE_NUM = atoi(argv[i_zyt+1]);
*/
	if(strcmp(argv[i_zyt],"-LOG")==0)
	{
	    LOG_LS_TRACE = 1;
	}
//        if(strcmp(argv[i_zyt],"-SET_NUM")==0)
//            SET_NUM = atoi(argv[i_zyt+1]);
//        if(strcmp(argv[i_zyt],"-SIZE")==0)
//            L1D_SIZE = atoi(argv[i_zyt+1]);
//        if(strcmp(argv[i_zyt],"-CACHE_LINE")==0)
//            CACHE_LINE = atoi(argv[i_zyt+1]);
//        if(strcmp(argv[i_zyt],"-ASSOCIATE")==0)
//            ASSOCIATE = atoi(argv[i_zyt+1]);
    }
/*
    printf("%d\n",argc);
    if(strcmp(argv[2],"LOG1")==0)
	LOG_LS_TRACE = 1;
    if(strcmp(argv[2],"LOG0")==0)
	LOG_LS_TRACE = 0;
*/
