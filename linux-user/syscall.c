...
#define MMAP_SHIFT 12
...
#ifdef __NR_exit_group
        /* new thread calls */
    case TARGET_NR_exit_group:    //main exit! ZYT
#ifdef TARGET_GPROF
        _mcleanup();
#endif
        gdb_exit(cpu_env, arg1);
	//****************************ZYT
        PRINT_CONF_STATS;
        print_SET_LIST(SET_NUM_LIST); 
    if(LOG_QEMU_RDD)
        print_RDD(HIS_RDD, HIS_sample_RDD, SET_NUM_LIST);
//#endif
    if(LOG_QEMU_SDD)
        print_SDD(HIS_SDD, HIS_sample_SDD, SET_NUM_LIST);
//#endif
        get_time();
        //****************************ZYT
